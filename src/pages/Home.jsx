...
import { Search, LogIn, User as UserIcon, LayoutDashboard } from 'lucide-react'; // LayoutDashboard ditambah
...
export default function Home() {
  ...
  // 1. Cek User Session + Cek Role (PENTING)
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex gap-4">
            {user ? (
              ...
              <div className="flex items-center gap-3">
                ...
                {user.role === 'admin' && (
                  <Link to="/admin" className="flex items-center gap-2 bg-gray-900 text-white px-4 py-2 rounded-full font-bold hover:bg-black transition shadow-sm border border-gray-700 text-sm">
                    <LayoutDashboard size={16} /> Panel Admin
                  </Link>
                )}

                {/* Tombol Dashboard Penjual (Muncul untuk semua member) */}
                <Link to="/dashboard" className="flex items-center gap-2 bg-genpro-orange text-genpro-maroon px-4 py-2 rounded-full font-bold hover:bg-yellow-400 transition shadow-sm text-sm">
                  <UserIcon size={16} /> Dashboard Saya
                </Link>
              </div>
            ) : (
              // Jika BELUM login
              <Link to="/login" className="flex items-center gap-2 text-gray-600 hover:text-genpro-orange transition-colors font-medium">
      ...
      {/* --- HERO SECTION --- */}
      <div className="bg-genpro-maroon text-white py-16 px-4 text-center relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h1 className="text-4xl md:text-5xl font-bold mb-4">Beli di Tetangga, <br/><span className="text-genpro-orange">Rezeki Tetap Terjaga</span></h1>
      ...
      <div className="max-w-6xl mx-auto p-4 mt-8 pb-20">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-genpro-maroon border-b-4 border-genpro-orange inline-block pb-1">
              ...
              <Link to={`/product/${item.id}`} key={item.id} className="block group">
                <div className="bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 h-full flex flex-col">
                  ...
                  <div className="relative h-48 overflow-hidden bg-gray-100">
                    ...
                    />
                    <div className="absolute top-2 right-2 bg-white/90 px-2 py-1 rounded text-xs font-bold text-gray-600 shadow-sm backdrop-blur-sm">
                      Stok: {item.stok}
                    </div>
                  </div>
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    ...
                    <h3 className="font-bold text-gray-800 truncate text-lg mb-1" title={item.nama_produk}>
                      {item.nama_produk}
                    </h3>
                    <p className="text-genpro-orange font-bold text-xl mt-auto">
